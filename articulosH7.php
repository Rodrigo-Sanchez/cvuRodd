<?php
	...
	$ptotales = count(getCapitulosHumanindex($nt));
...
?>
	...
	<div class="row">
		...
		<div class="col-md-9" style="background-color:white; padding-bottom:50px; ">
			...
			<div class="row">
				<div class="col-md-12">
					<center><h4><b><?= $datos[0]['NombreCompleto'] ?></b></h4><h4>Humanindex - <i> Capítulos de libro </i></h4></center>
			...
			<?php
				...
				$lista = getCapitulosHumanindex($nt);
					...
					<input type="hidden" name="nt" value="<?= $nt ?>" />
						...
						<table id="example" style="font-size:12px;  " class="display" cellspacing="0" >
							...
							foreach($lista as $l) {
								...
								$autores = getAutoresCapsHumanindex($identificador);	
								$cuentaAutores=1;
								...
								<tr>
									<td><?= $contador ?></td>
									<td><?= utf8_encode($l['titulo_libro']) ?></td>
									<td><?= utf8_encode($l['titulo_capitulo']) ?></td>
									<td><?= $cadenaAutores ?></td>
