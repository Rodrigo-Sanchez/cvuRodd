<?php
	...
	$ptotales = count(getArticulosHumanindex($nt));
...
?>
	...
	<div class="row">
		...
		<div class="col-md-9" style="background-color:white; padding-bottom:50px; ">
			...
			<div class="row">
				<div class="col-md-12">
					<center><h4><b><?= $datos[0]['NombreCompleto'] ?></b></h4><h4>Humanindex - <i> Artículos </i></h4></center>
			...
			<?php
				...
				$lista = getArticulosHumanindex($nt);
					...
					<input type="hidden" name="nt" value="<?= $nt ?>" />
						...
						<table id="example" style="font-size:12px;  " class="display" cellspacing="0" >
							...
							foreach($lista as $l) {
								...
								$autores = getAutoresHumanindex($identificador);	
								$cuentaAutores=1;
								...
								<tr>
									...
									<td><?= $cadenaAutores ?></td>
									<td><?= utf8_encode($l['revista']) ?></td>
									<td><?= utf8_encode($l['issn']) ?></td>
									<td><?= $l['anno'] ?></td>
