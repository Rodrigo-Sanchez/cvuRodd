<?php
	...
	$ptotales = count(getListaPublicacionesLibrosInd($nt));
...
?>
	...
	<div class="row">
		...
		<div class="col-md-9" style="background-color:white; padding-bottom:50px; ">
			...
			<div class="row">
				<div class="col-md-12">
					<center><h4><b><?= $datos[0]['NombreCompleto'] ?></b></h4><h4>Documentos indexados WoS, Scopus y Pubmed - <i>Libros</i> </h4></center>
			...
			<?php
				$lista = getListaPublicacionesLibrosInd($nt);
			?>
			<div class="row">
					...
					<input type="hidden" name="nt" value="<?= $nt ?>" />
						...
						<table id="example" style="font-size:12px;  " class="display" cellspacing="0" >
							...
							foreach($lista as $l) {
								...
								<tr>
									<td><?= $contador ?></td>
									<td><p ><a href="https://produccion.siia.unam.mx/Publicaciones/ProdCientif/PublicacionFrw.aspx?scopus=0&id=<?=$identificador?>" target="_blank"><?= utf8_encode($l['ObraTitulo']) ?></a></p> <p style="color:blue;" onclick="ponFrame(<?= $identificador ?>)" >- Ver detalles</p></td>
